...
template <typename T>
typename set<T>::node *set<T>::leftRotate(node *h) {
    node *x = h->right;
    h->right = x->left;
    x->left = h;
...
template <typename T>
typename set<T>::node *set<T>::rightRotate(node *h) {
    node *x = h->left;
    h->left = x->right;
    x->right = h;
...
template <typename T>
typename set<T>::node *set<T>::colorFlip(node *h) {
    h->color = !h->color;
...
template <typename T>
typename set<T>::node *set<T>::fixUp(node *h) {
    if (isRed(h->right)) h = leftRotate(h);

    if (isRed(h->left) && isRed(h->left->left)) h = rightRotate(h);

    if (isRed(h->left) && isRed(h->right)) h = colorFlip(h);

    return h;
}

template <typename T>
typename set<T>::node *set<T>::moveRedRight(node *h) {
    colorFlip(h);
    if (h->left != nullptr) {
...
template <typename T>
typename set<T>::node *set<T>::moveRedLeft(node *h) {
    colorFlip(h);
    if (h->right != nullptr) {
...
template <typename T>
typename set<T>::node *set<T>::eraseMin(node *h) {
...
template <typename T>
typename set<T>::node *set<T>::erase(node *h, T value) {
    if (value < h->value) {
        if (!isRed(h->left) && !isRed(h->left->left)) h = moveRedLeft(h);
        h->left = erase(h->left, value);
    } else {  // value >= h->value
        if (h->left != nullptr)
            if (isRed(h->left)) h = rightRotate(h);
        if (value == h->value && h->right == nullptr) return nullptr;
        if (h->right != nullptr)
...
template <typename T>
bool set<T>::isRed(node *h) {
    if (h == nullptr) return false;
    return h->color == RED;
}

template <typename T>
void set<T>::freeSubtree(node *h) {
    if (h == nullptr) return;
    if (h->left != nullptr) freeSubtree(h->left);
    if (h->right != nullptr) freeSubtree(h->right);
    delete h;
}
