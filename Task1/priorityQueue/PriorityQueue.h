...
template <typename T> void priority_queue<T>::consolidate() {
  ...
  while (cur_node != nullptr) {
    ...
    if (tree_size[log_degree] == nullptr) {
      ...
      if (_old->value < _new0->value) {
        if (_old->child != nullptr)
          _old->child->parent = _new0;
        if (_new0->child != nullptr)
          _new0->child->parent = _old;
      ...
      if (_old->value < consol.min_node->value)
        consol.min_node = _old;
      if (_new0->value < consol.min_node->value)
      ...
      while (tree_size[log_degree] != nullptr) {
        ...
        if (_new1->left != nullptr)
          _new1->left->right = _new1->right;
        if (_new1->right != nullptr)
          _new1->right->left = _new1->left;
...
template <typename T> bool priority_queue<T>::empty() const {
  return size_ == 0;
}

template <typename T> size_type priority_queue<T>::size() const {
  return size_;
}
